...
export function createState(inParentComponent: React.Component) {
    return {
        //trenutni board layout
        //tijekom igre, -1 zamijeni 1 kad se plocice ociste
        //kada su odabrane, postaju 1001-1042

        layout: <number[][][]>[],

        //prati koje su plocice odabrane
        selectedTiles: <ISelectedTile[]>[],

        //trenutacni rezultat
        scores: <IScores>{ player: 0, opponent: 0 },

        //u kojoj je trenutno fazi igra
        ...
        //socketComm objekt za server pozive
        socketComm: <Function>createSocketComm(inParentComponent),

        //proteklo vrijeme od zadnjeg matchanja para plocica
        ...
        handleMessage_gameOver: function (inPID: string) {
            if (inPID === this.state.pid) {
                this.setState({ gameState: "gameOver", gameOutcome: "Cestitam, pobijedili ste" });
            } else {
                this.setState({ gameState: "gameOver", gameOutcome: "Nazalost, izgubili ste" });
            }
        }.bind(inParentComponent),


        //--------------------------game functions-------------------------------
        //inLayer - broj layera gdje se nalazi plocica

        tileClick: function (inLayer: number, inRow: number, inColumn: number) {
            //prekid ukoliko nije u play stateu
            if (this.state.gameState !== "playing") {
                return;
            }

            //provjeri moze li se plocica odabrati, ako ne moze prekini
            if (!this.state.canTileBeSelected(inLayer, inRow, inColumn)) {
                return;
            }

            //pronadji trenutnu vrijednost odabrane plocice
            const layout: number[][][] = this.state.layout.slice(0);
            const currentTileValue: number = layout[inLayer][inRow][inColumn];

            //samo smiju odabrati plocice, a ne prazan prostor i/ili uklonjene plocice
            ...
            //plocica je trenutno oznacena, odznaci je i ukloni status odabrana
            if (currentTileValue > 1000) {
                ...
                //trenutno nije oznaceno, oznaci i spremi kao odabrano
            } else {
                layout[inLayer][inRow][inColumn] = currentTileValue + 1000;
                selectedTiles.push({ layer: inLayer, row: inRow, column: inColumn, type: currentTileValue });
            }

            //ukoliko su 2 plocice odabrane, provjeri jesu li par. Ako jesu, ukloni plocice i povecaj rezultat
            //Ukoliko nisu, odznaci ih i ocisti selectedTiles. To se odnosi i na wildcard(101) plocicu
            if (selectedTiles.length === 2) {
                ...
                if (selectedTiles[0].type === selectedTiles[1].type ||
                    ...
                    //izracunaj bodove koje ce igrac dobiti
                    ...
                    //sibni rezultat na server
                    this.state.socketComm.send(`match_${this.state.pid}_${calculatedPoints}`);

                    //provjeri da li je ploca ociscena ili nema vise poteza
                    ...
                    switch (anyMovesLeft) {
                        ...
                        case "cleared":
                            scores.player += 100;
                            gameState = "cleared";
                            this.state.socketComm.send(`match_${this.state.pid_100}`);
                            this.state.socketComm.send(`done_${this.state.pid}`);
                            break;
                    }
            ...
            //postavi state da se plocice ponovno izvuku
            this.setState({
                gameState: gameState,
                layout: layout,
        ...
        //projera moze li se dana plocica odabrati po pravilima igre, tj. plocica ne smije imati plocicu na vrhu
        //uzimajuci u obzir clickHandler, to ne bi trebalo biti moguce, te mora biti slobodna s barem jedne strane
        canTileBeSelected: function (inLayer: number, inRow: number, inColumn: number): boolean {
            //ako je plocica u gornjem sloju ili ne postoji plocica povrh nje te ako je u prvom retku, zadnjem retku
            //ili nema sa strane druga plocica, onda je slobodna za odabir
        ...
        //odreduje da li je ostao najmanje jedan potez koji igrac moze napraviti
        anyMovesLeft: function (inLayout: number[][][]): string {
            //za pocetak, pronadji sve slobodne plocice (one koje se mogu odabrati) i istovremeno prebroji koliko je poteza ostalo
                    ...
                    for (let c: number = 0; c < row.length; c++) {
                        ...
                        if (tileVal > 0) {
                            ...
                            if (this.state.canTileBeSelected(l, r, c)) {
                                //ukoliko je jedna od njih wildcard, short-circuit jer preostaje barem jedan potez kada se tu nalazi wildcard
                                if (tileVal === 101) {
                                    return "yes";
                                }
                                //u suprotnome, nastavi provjeravati i spremi odabrani plocicu
            ...
            //ukoliko nema plocica, znaci da je ploca ociscena
            if (numTiles === 0) {
                return "cleared";
            }

            //prolazi kroz plocice koje se mogu odabrati i broji koliko se puta mogu pojaviti
            const counts: number[] = [];
            ...
            //ovaj korak govori da nije preostao vise niti jedan par, igra se ne moze nastaviti
            return "no";
        }.bind(inParentComponent)
